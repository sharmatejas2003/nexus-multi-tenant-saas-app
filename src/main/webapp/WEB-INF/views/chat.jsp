...
<head>
    ...
    <style>
        ...
        @media (max-width:900px) { .chat-layout { grid-template-columns:1fr; height:auto; } .sidebar-panel { display:none; } }
    </style>
</head>
<body>
...
<div class="main" style="padding-bottom:0;">
    <div class="topbar" style="margin-bottom:16px;">
        <div>
            ...
            <div class="page-subtitle">
                ...
                <span class="online-dot"></span> Team workspace
            </div>
        </div>
    ...
    <c:if test="${param.announced == 'true'}">
        <div class="alert alert-success" style="margin-bottom:14px;">📢 Announcement posted and team notified!</div>
    </c:if>
    ...
    <div class="chat-layout">
        ...
        <div class="chat-main">
                ...
                <c:choose>
                    <c:when test="${empty messages}">
                        <div style="text-align:center;padding:40px 20px;color:var(--text2);">
                            ...
                            <p style="font-size:14px;font-weight:600;margin-bottom:6px;">Start the conversation!</p>
                            <p style="font-size:13px;">Send a message to your team below.</p>
                        </div>
                    </c:when>
                    <c:otherwise>
                                ...
                                <div class="msg-time">
                                    ...
                                    <c:if test="${isAdminOrOwner}">
                                        <span style="margin-left:8px;cursor:pointer;color:var(--accent2);font-size:9px;"
                                              onclick="deleteMsg(${msg.id})">✕ delete</span>
                                    </c:if>
                                </div>
                            </div>
                        </div>
            ...
            <div class="chat-input-area">
                <div class="typing-indicator" id="typingIndicator">Teammate is typing...</div>
                <div class="chat-input-row">
                    <textarea class="chat-input" id="msgInput" placeholder="Message #general... (Enter to send, Shift+Enter for newline)"
                              rows="1" maxlength="2000" oninput="onType(this)"></textarea>
                    <button class="chat-send-btn" onclick="sendMessage()" title="Send message">➤</button>
                </div>
        ...
        <div class="sidebar-panel">
            ...
            <div class="card" style="padding:16px;">
                <div class="card-header" style="margin-bottom:12px;">
                    <span class="card-title" style="font-size:13px;">📌 Pinned</span>
                    <c:if test="${isAdminOrOwner}">
                        <button class="btn btn-ghost btn-sm"
                                onclick="document.getElementById('announceModal').classList.add('open')">+ Add</button>
                    </c:if>
                </div>
                <c:choose>
                    ...
                    <c:otherwise>
                        ...
                        <div class="announce-card ${a.pinned ? 'announce-pinned' : ''}" style="margin-bottom:8px;">
                            <div class="announce-priority priority-${a.priority}">${a.priority}</div>
                            <c:if test="${a.pinned}">
                                <span style="font-size:10px;color:var(--accent);">📌 Pinned &nbsp;</span>
                            </c:if>
                            <div class="announce-title">${a.title}</div>
                            <c:if test="${not empty a.content}">
                                <div class="announce-body">${a.content}</div>
                            </c:if>
                            <div style="font-size:10px;color:var(--text2);margin-top:6px;display:flex;justify-content:space-between;align-items:center;">
                                <span>${a.createdBy} · ${a.timeAgo}</span>
                                <c:if test="${isAdminOrOwner}">
                                    <form action="/announcements/delete/${a.id}" method="post" style="display:inline;">
                                        <button class="btn btn-danger" style="padding:2px 6px;font-size:10px;">✕</button>
                                    </form>
                                </c:if>
                            </div>
                        </div>
    ...
    <div class="modal" style="width:500px;">
        ...
        <form action="/announcements/add" method="post">
            ...
            <div class="form-group">
                ...
                <textarea name="content" class="form-textarea" rows="3" placeholder="Provide additional details..."></textarea>
            </div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px;">
                ...
                <div style="display:flex;align-items:flex-end;padding-bottom:4px;">
                    <label style="display:flex;align-items:center;gap:8px;cursor:pointer;">
                        <input type="checkbox" name="pinned" value="true" style="width:16px;height:16px;accent-color:var(--accent);">
                        <span class="form-label" style="margin:0;">📌 Pin this</span>
                    </label>
                </div>
            </div>
            <div style="display:flex;gap:10px;justify-content:flex-end;">
                <button type="button" class="btn btn-ghost"
                        onclick="document.getElementById('announceModal').classList.remove('open')">Cancel</button>
...
function pollMessages() {
    ...
    .then(msgs => {
        if (msgs.length > lastMsgCount) {
            // New messages arrived, reload new ones
